class Api::V2::QualifiedValuesController < Api::V1::BaseController
    ...
    def index
      if params[:token].present? && params[:buoy].present?
        ...
        buoy = MooredBuoy.where("buoy_id = ?", params[:buoy])
        unless user.empty?
          unless buoy.empty?
            @query = ""
            ...
            if !@query.downcase.match(/(\/|;|drop|\*|if|\+|\-\-|\!|concat|char|union)/).to_a.empty?
              ...
              if params[:limit].present?
                @qualified_values = QualifiedValue.where(@query).order(date_time: :desc).limit(params[:limit].to_i)
              else
                @qualified_values = QualifiedValue.where(@query).order(date_time: :desc)
              end
            end
          end
        end
    ...
    def show
      @qualified_value = QualifiedValue.where("id = params[:id]")
