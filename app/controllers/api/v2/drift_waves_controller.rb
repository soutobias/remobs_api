class Api::V2::DriftWavesController < Api::V1::BaseController
    ...
    def index
      if params[:token].present? && params[:buoy].present?
        ...
        buoy = DriftBuoy.where("buoy_id = ?", params[:buoy])
        unless user.empty?
          @query = ""
          ...
          if !@query.downcase.match(/(\/|;|drop|\*|if|\+|\-\-|\!|concat|char|union)/).to_a.empty?
            ...
            if params[:limit].present?
              @qualified_values = DriftWave.where(@query).order(date_time: :desc).limit(params[:limit].to_i)
            else
              @qualified_values = DriftWave.where(@query).order(date_time: :desc)
            end
          end
        end
      end
    ...
    def show
      @qualified_value = DriftWave.where("id = params[:id]")
